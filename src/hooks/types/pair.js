...
import { FACTORY_ADDRESSES, INIT_CODE_HASH, USDD_ADDRESS, FEE, FEE_BASE, POOLTOKEN_DECIMALS } from '../../constants/index'
...
export default class Pair extends UPair {
  ...
  getOutputAmount(inputAmount) {
    ...
    if (isSameAddress(inputAmount.token.address, USDD_ADDRESS)) {
      ...
      outputAmount = new TokenAmount(
        this.token0,
        JSBI.divide(JSBI.multiply(JSBI.divide(numerator, denominator), _fee), _feeBase)
      )
      if (JSBI.equal(outputAmount.raw, ZERO)) {
        throw new InsufficientInputAmountError()
      }
  ...
  getInputAmount(outputAmount) {
    ...
    if (isSameAddress(outputAmount.token.address, USDD_ADDRESS)) {
      ...
      inputAmount = new TokenAmount(
        this.token0,
        JSBI.multiply(JSBI.divide(JSBI.divide(numerator, denominator), _fee), _feeBase)
      )
    }
