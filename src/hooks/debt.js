...
export async function getAllDebt(chainId, pairAddress, token) {
  ...
  for (const l of logs) {
    ...
    if (data.debtToken0Amount.isZero() && data.repayAmount.isZero()) {
      dataMap.delete(key)
      continue
    }
    const parsedData = {
      ...
      debtToken0Amount: new TokenAmount(USDD, data.debtToken0Amount),
      debtToken1Amount: new TokenAmount(token, data.debtToken1Amount),
      owner: data.owner,
...
export function getInfoAfterRemoveLpConsiderLiquidation(pair, totalSupply, removeAmount, debtsInfo) {
  ...
  const [tokenExtraRec, usddSpend, usddDebtAmount, impactNum, overlapUsdd] = overlapDebit(JSBI.subtract(y, usddOri), JSBI.subtract(x, tokenOri), debtsInfo)
  return [JSBI.subtract(JSBI.subtract(usddOri, usddSpend), usddDebtAmount).toString(), JSBI.add(tokenOri, tokenExtraRec).toString(), usddDebtAmount.toString(), impactNum, overlapUsdd]
}
...
function overlapDebit(_reserve0, _reserve1, debtInfos) {
  ...
  for (let i = debtInfos.length - 1; i >= 0; i--) {
    ...
    if (JSBI.equal(JSBI.BigInt(0), _tmpStartX) || clearAll) {
      ...
      overlapUsdd = JSBI.add(overlapUsdd, info.repayAmount.raw);
      impactDebitNum += 1;
      clearAll = true;
      continue
    }
    if (JSBI.lessThan(_tmpStartX, _reserve1)) {
      if (JSBI.lessThanOrEqual(JSBI.add(_tmpStartX, info.pledgeAmount.raw), _reserve1)) {
        _getToken1Amount = JSBI.add(_getToken1Amount, info.pledgeAmount.raw)
        _payToken0Amount = JSBI.add(_payToken0Amount, info.repayAmount.raw)
        info.pledgeAmount = new TokenAmount(info.pledgeAmount.token,JSBI.BigInt(0))
        info.repayAmount = new TokenAmount(info.repayAmount.token, JSBI.BigInt(0))
        overlapUsdd = JSBI.add(overlapUsdd, info.repayAmount.raw);
        impactDebitNum += 1
        continue
      }
      ...
      _tmpToken0 = JSBI.divide(JSBI.multiply(_reserve0, _tmpToken1), _tmpStartX);
      if (JSBI.lessThan(info.pledgeAmount.raw, _tmpToken1) || JSBI.lessThan(info.repayAmount.raw, _tmpToken0)) {
        _tmpToken1 = info.pledgeAmount.raw
        _tmpToken0 = info.repayAmount.raw
      }
      _getToken1Amount = JSBI.add(_getToken1Amount, _tmpToken1);
      _payToken0Amount = JSBI.add(_payToken0Amount, _tmpToken0);
      overlapUsdd = JSBI.add(overlapUsdd, _tmpToken0);
  ...
  return [_getToken1Amount, _payToken0Amount, _refundToken0Amount, impactDebitNum, overlapUsdd];
}
...
export function getUSDDYMapping(usddReserve, tokenReserve, debtInfo) {
  ...
  for (const d of debtInfo.sortByUSDDPoint(usddReserve.raw, tokenReserve.raw)) {
    const [, ybegin, , yend] = liquidationPoint(tokenReserve.raw, usddReserve.raw, d.pledgeAmount.raw, d.repayAmount.raw)
    clearingList.push({
      minVal: yend,
      maxVal: ybegin,
      disabled: true
