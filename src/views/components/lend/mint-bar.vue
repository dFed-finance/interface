...
export default class MintItem extends Vue{
  ...
  @Watch('tokenAddress')
  tokenAddressChange(nval,oval){
    this.sliderBarCalculate(true)
  }
  ...
  sliderBarCalculate(isChangeAddress){
    if(!this.usddValue){
      return
    }
    ...
    if(isChangeAddress){
      ...
      this.sliderBarWidth = sliderBarWidthTemp <= 0 ? 0 : sliderBarWidthTemp;
  ...
  handleMousemove(e){
    ...
    }else if(sliderBarRightTemp >= this.lineWidth){
      this.sliderBarRight = this.lineWidth;
    }else{
