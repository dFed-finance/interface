  ...
  <div class="remove-liquidity">
    ...
    <div class="remove-content">
      ...
      <div class="token-count-wrap">
        ...
        <DescComponent :title="'Credit('+tokenA.symbol+'):'" :value="creditValue || '-'" />
...
export default class RemoveLiquidity extends Vue {
  ...
  percentValueChange() {
    ...
    this.totalValue = this.pair.reserve0.toSignificant();
    // overlapping
    if (!this.debtInfo.isEmpty()) {
      const debtInfo = this.debtInfo;
      const sortedDebts = [].concat(debtInfo.sortByTokenPoint(
      ...
      const [getUSDD, getToken, usddDebtAmount, impact] = res;
      ...
      this.creditValue = new TokenAmount(
        this.usddToken,
        usddDebtAmount
      ).toSignificant();
    } else {
