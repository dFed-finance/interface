        ...
        <div class="dialog-bg-desc-content">
          ...
          <p class="flex-row-between-start p_t5 p_b5">
            ...
            <span class="flex-colum-center-end">
              <span>1 {{exchangeRate.name2}} = {{exchangeRate.value2}} {{exchangeRate.name1}}</span>
              <span>1 {{exchangeRate.name1}} = {{exchangeRate.value1}} {{exchangeRate.name2}}</span>
            </span>
          </p>
          <p class="flex-row-between-center p_t5 p_b5">
...
export default class AddLiquidity extends Vue {
  ...
  computeMint() {
    ...
    try {
      ...
      this.$message.error("Insufficient reserves")
  ...
  handleChangeData(field, val) {
    ...
    if (!this.validateInput(val, field)) {
      return;
    }
  ...
  validateInput(inputValue, fieldName) {
    ...
    if (tokenName === this.usddToken.symbol) {
      try {
        tryParseAmount(inputValue, this.usddToken);
      } catch (e) {
      ...
      try {
        tryParseAmount(inputValue, this.otherToken);
      } catch (e) {
  ...
  isApproving(tokenAddress) {
    ...
    return tokenAddress.toLowerCase() === this.usddToken.address.toLowerCase()
